...
`npm start` 会启动 `server.mjs`。服务端创建浏览器会话、持久化任务/候选/审计事件，并在新任务创建和定时扫描时重算双方活跃任务。前端只负责输入、材料选择、状态展示和本人确认；预算上限、最低授权价、精确地址和上传原件不会进入对方公开候选。默认结构化使用确定性解析；配置 SiliconFlow Key 后，模型只负责补充自然语言字段，规则结果优先，不能覆盖已经明确的硬事实。

这套本地实现用于验证产品闭环，不等同于生产实名核验：SQLite 代替 PostgreSQL，本地目录代替私有对象存储，轮询代替队列和推送，种子市场代替真实供需。迁移生产时要保留同样的 `owner_id` 隔离、服务端证据引用校验和公开字段脱敏边界。
